...
/**
 * The account-servicing-agent-api backend.
 *
 ...
 * `registerAuth(workspace("account-servicing-agent-api"), { canonical: "authn" })` returns the
 * instance to chain your own .register*() calls onto:
 *
 *   registerAuth(workspace("account-servicing-agent-api"), { canonical: "authn" })
 *     .registerTables([notes])
 *     .registerApiGroups([api])
 *     .registerQueries([createNote]);
 *
